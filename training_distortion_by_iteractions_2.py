...
if __name__ == '__main__':
    ...
    #num_levels_set = [64] #, 8, 16, 32, 64, 128, 256, 512]
    num_levels_set = [4, 8, 16, 32, 64, 128, 256, 512]
    
    dataset_name = 's002'

    fig, ax = plt.subplots(4,2, figsize=(10,10), gridspec_kw = {'wspace':0.025, 'hspace':0.15}, sharey=True)
    ...
    for n in n_set:
        ...
        marker_dict = {'random': "*", 'random_from_samples': "h", 'xiaoxiao': "p"}
        ...
        for filter_id, mean_distortion in mean_distortion_dict.items():
            ...
            ax[ax_pos_dict[user_filter_set[filter_id]['num_of_levels']]].plot(10 * np.log10(mean_distortion), marker=marker_dict[user_filter_set[filter_id]['initial_alphabet_opt']], color=color_dict[user_filter_set[filter_id]['initial_alphabet_opt']], label=f"{label}, L={user_filter_set[filter_id]['num_of_levels']}")
            ax[ax_pos_dict[user_filter_set[filter_id]['num_of_levels']]].legend(loc='best', fontsize='9')
            ax[ax_pos_dict[user_filter_set[filter_id]['num_of_levels']]].grid(True)
            #ax[ax_pos_dict[user_filter_set[filter_id]['num_of_levels']]].plot(10 * np.log10(mean_distortion), marker=marker_dict[user_filter_set[filter_id]['num_of_levels']],  color=color_dict[user_filter_set[filter_id]['initial_alphabet_opt']], label=f"{label}, L={user_filter_set[filter_id]['num_of_levels']}")
        ...
        #fig.supxlabel(f'Número de iterações', fontsize='11')
        #fig.supylabel(r'Distorção média $d_m$ em db', fontsize='11') 
        fig.text(0.5, 0.05, 'Número de iterações', ha='center')
        fig.text(0.05, 0.5, r'Distorção média $d_m$ em db', va='center', rotation='vertical')
