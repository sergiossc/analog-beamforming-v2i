...
#        #    rx_array_vec = np.arange(rx_size)
#
#        #    at = np.array([np.exp(-1j * 2 * np.pi * n * tx_spacing * (1/tx_wavelength) * np.cos(aod_theta)) for n in range(len(tx_array_vec))])
#        #    at = at * (1/np.sqrt(len(tx_array_vec)))
#
#        #    ar = np.array([np.exp(-1j * 2 * np.pi * n * rx_spacing * (1/rx_wavelength) * np.cos(aoa_theta)) for n in range(len(rx_array_vec))])
