...
for i in range(num_paths):
    ...
    #phase = np.random.choice([1, 10e-1, 10e0, 10e-3, 10e-2], 1, replace=False)
    phase = 2*np.pi
...
num_tx_z = int(np.sqrt(tx_array.size)) # number of elelement of tx at y_axis
# @ RX
...
#
#### Once we have the CSI (h), let's thing about the transmition and derivate precoder using SVD
...
##print ('--------------------------------------')
#w = vh[0]
#w = np.array(w).reshape(num_tx_x, num_tx_y)
#w = w.conj().T
#print ('w: \n', w)
#print ('abs.w: \n', np.abs(w))
#print ('angle.w(deg): \n', np.rad2deg(np.angle(w)))
###pattern3d(tx_array, w)
##w = h
##unitary_w = np.exp(1j * np.angle(w))
