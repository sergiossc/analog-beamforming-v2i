                    ...
                    if len(paths) > 0: # this is a valid channel there is at least one ray
                        ...
                        for p in paths:
                            if p['los'] > 0.0:
                                ...
                                #print (type(p['los']))
                                #print (p['los'])
                        sample_set[sample_id] = {'channel_matrix': h, 'ep_id': ep_id, 'is_there_los_paths': is_there_los_paths, 'receiver_id': receiver_id}
#
    print (f'%num_of_valid_channels: {num_of_valid_channels}')
    return sample_set


def get_paths_by_receiver(csvsetfile, pathfiles=None):
    paths_set = {i+1: [] for i in range(10)}
    #paths = []
    num_of_valid_channels = 0
        ...
        for ep_id, scenes in episode.items():
            ...
            for scene_id, scene in scenes.items():
                ...
                for chn_id, channel in scene.items():
                    ...
                    receiver_id = receiver_id+1
                    for path_id, path in channel.items():
                        paths_set[receiver_id].append(path)
                        #paths.append(path)
                    #print (f'---------------> len(paths) by channel: {len(paths)}')
                    #if len(paths) > 0: # this is a valid channel there is at least one ray
                    #    num_of_valid_channels += 1
                    #    receiver_id = receiver_id+1
                    #    #plot_paths(path_scene, chn_paths, tx_transceiver, rx_transceiver)
                    ...
                    #            #print (type(p['los']))
                    #            #print (p['los'])
                    #    #paths_set[receiver_id].appen(paths) = {'paths': paths, 'ep_id': ep_id, 'is_there_los_paths': is_there_los_paths, 'receiver_id': receiver_id}
    return paths_set


def get_paths_by_receiver_by_channel(csvsetfile, pathfiles=None):
    sample_set = {}
    num_of_valid_channels = 0
                    ...
                    if len(paths) > 0: # this is a valid channel there is at least one ray
                        ...
                        for p in paths:
                            if p['los'] > 0.0:
                                ...
                                #print (type(p['los']))
                                #print (p['los'])
                        sample_set[sample_id] = {'paths': paths, 'ep_id': ep_id, 'is_there_los_paths': is_there_los_paths, 'receiver_id': receiver_id}
#
    print (f'%num_of_valid_channels: {num_of_valid_channels}')
    return sample_set
