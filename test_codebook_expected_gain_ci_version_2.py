    ...
    for dataset_name in dataset_name_set:
        ...
        for n in n_set:
            ...
            for num_levels in num_levels_set:
                ...
                # DFT
                dft_oversampling_factor += 1 
                fftmat, ifftmat = fftmatrix(n_tx, 2**dft_oversampling_factor)
                ...
                bf_gain_mean_dft[len(cb_dft)] = np.mean(bf_gain_dft)
            ...
            color_dict_l = {4: 'cornflowerblue', 8: 'navy', 16: 'mediumblue', 32: 'darkviolet', 64: 'violet', 128: 'teal', 256:'olive', 512: 'coral' }
            ...
            for df_col in df.columns:
                if df_col != 'opt':
                    ...
                    x = np.array(df[df_col])
                    x_set.append(x)
                    
                    sample_mean, ci_values = conf_interval(x, conf_level=0.90)
                    ci_values_set.append(ci_values)
                    labels_set = df_col
                    #print (f'sample_mean: {sample_mean}')
                    #print (f'ci_values: {ci_values}\n')
                    #y.append(sample_mean)
                    #yerr.append((sample_mean - ci_values[1]) + (sample_mean - ci_values[0]))


            ax.boxplot(x_set, conf_intervals=ci_values_set, notch=True)
            #plt.xticks(np.arange(len(x_set)), labels_set)
            ...
            #bf_gain_mean_dft_keys_sorted = sorted(bf_gain_mean_dft)
            #dft_colors=plt.cm.get_cmap('twilight', 5*len(bf_gain_mean_dft_keys_sorted))
            #for k in bf_gain_mean_dft_keys_sorted:
            #    print (f'k: {k}')
            #    if k > 512:
            #        pass 
            #    else:
            #        num_of_dft_cw = k
            #        ax.plot(range(len(num_levels_set)), np.ones(len(num_levels_set)) * bf_gain_mean_dft[k], color=color_dict_l[k], label=f"{initial_alphabet_set_label_pt_dict['dft']}, L={num_of_dft_cw}", marker=marker_dict['dft'])
            ...
            #plt.grid(True)
            #image_filename = f'test-bf-expected-gain-{dataset_name}-n{n}-ci-version.png'
