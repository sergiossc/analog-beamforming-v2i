...
from utils import get_frab_vec
...
if __name__ == "__main__":
    ...
    for _ in range(num_samples):
        ...
        B = 8 # number of bits on frab
        Nq = 2 ** B # number of values
        f_frab = (1/np.sqrt(num_tx)) * np.matrix(get_frab_vec(f_est, Nq))
        
        

        w = np.matrix(u.conj()[:,0]) # vetor coluna: Nx1
        w_est = (1/np.sqrt(num_rx)) * np.exp(1j * np.angle(w))

        print (f'\n\nAnalyzing procoder/combining: \n')

        print (f'np.shape(f): {np.shape(f)}')
        print (f'np.shape(f_est): {np.shape(f_est)}')
        print (f'np.shape(f_frab): {np.shape(f_frab)}')
        print (f'norm(f): {norm(f)}')
        print (f'norm(f_est): {norm(f_est)}')
        print (f'norm(f_frab): {norm(f_frab)}')
        print (f'f: \n{f}')
        print (f'f_est: \n{f_est}')
        ...
        prod_frab = np.dot(w, np.dot(h, f_frab.T))
        gain_bf_frab.append(norm(prod_frab[0,0]) ** 2)

    gain_channel = np.array(gain_channel)
    gain_bf_opt = np.array(gain_bf_opt)
    gain_bf_est = np.array(gain_bf_est)
    gain_bf_frab = np.array(gain_bf_frab)
    ...
    plt.plot(gain_bf_frab, label='frab bf gain')
