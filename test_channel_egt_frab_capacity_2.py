...
if __name__ == "__main__":
    ...
    for snr_v in snr:
        ...
        for ch_id in ch_id_list:
            ...
            f_egt_frab_b4 = get_frab(cb_dict_egt[cw_id_tx], b)
            gain_est_egt_b4 = bf_gain_test(ch,  f_egt_frab_b4   , w_opt) # opt receiver
#            #print (f'rt_f_egt_frab_4b.norm: {norm(rt_f_egt_frab_4b)}')
#            rt_bf_gain_egt_frab_b4 = bf_gain_test(ch, rt_f_egt_frab_b4, rt_w_opt)
#            #bf_gain_rt_egt_frab_b4.append(rt_bf_gain_egt_frab_b4)
            bf_gain_rt_egt_frab_b4.append(np.log2(1 + snr_v *  gain_est_egt_b4))
#    
            b = 3
            #gain_est_egt_b3, cw_id_tx_b3 = beamsweeping2(ch, cb_dict_frab[b])
            f_egt_frab_b3 = get_frab(cb_dict_egt[cw_id_tx], b)
            gain_est_egt_b3 = bf_gain_test(ch,  f_egt_frab_b3   , w_opt) # opt receiver
#            rt_f_egt_frab_b3 = get_frab(rt_f_egt, b)
#            #print (f'rt_f_egt_frab_3b.norm: {norm(rt_f_egt_frab_3b)}')
#            rt_bf_gain_egt_frab_b3 = bf_gain_test(ch, rt_f_egt_frab_b3, rt_w_opt)
            bf_gain_rt_egt_frab_b3.append(np.log2(1 + snr_v *  gain_est_egt_b3))
#    
            b = 2
            #gain_est_egt_b2, cw_id_tx_b2 = beamsweeping2(ch, cb_dict_frab[b])
            f_egt_frab_b2 = get_frab(cb_dict_egt[cw_id_tx], b)
            gain_est_egt_b2 = bf_gain_test(ch,  f_egt_frab_b2   , w_opt) # opt receiver
#            rt_f_egt_frab_b2 = get_frab(rt_f_egt, b)
#            #print (f'rt_f_egt_frab_2b.norm: {norm(rt_f_egt_frab_2b)}')
#            rt_bf_gain_egt_frab_b2 = bf_gain_test(ch, rt_f_egt_frab_b2, rt_w_opt)
            bf_gain_rt_egt_frab_b2.append(np.log2(1 + snr_v *  gain_est_egt_b2))
#    
            b = 1
            f_egt_frab_b1 = get_frab(cb_dict_egt[cw_id_tx], b)
            gain_est_egt_b1 = bf_gain_test(ch,  f_egt_frab_b1   , w_opt) # opt receiver
            #gain_est_egt_b1, cw_id_tx_b1 = beamsweeping2(ch, cb_dict_frab[b])
            #rt_f_egt_frab_b1 = get_frab(rt_f_egt, b)
            #print (f'rt_f_egt_frab_b1.norm: {norm(rt_f_egt_frab_b1)}')
            #rt_bf_gain_egt_frab_b1 = bf_gain_test(ch, rt_f_egt_frab_b1, rt_w_opt)
...
#
#    
    plt.plot(snr_db, bf_gain_opt_egt_mean, label='RT - BF gain OPT (EGT)')
    plt.plot(snr_db, bf_gain_rt_egt_mean, label='RT - BF gain EST (EGT)')
    #plt.plot(bf_gain_random_opt, label='RANDOM - BF gain (OPT)')
    
    #plt.plot(bf_gain_random_egt, label='RANDOM - BF gain (EGT)', linestyle='--')
    
    plt.plot(snr_db, bf_gain_rt_egt_frab_4b_mean, label='RT BF gain EST (EGT) -- frab -- b=4', linestyle='--')
    plt.plot(snr_db, bf_gain_rt_egt_frab_3b_mean, label='RT BF gain EST (EGT) -- frab -- b=3', linestyle='--')
    plt.plot(snr_db, bf_gain_rt_egt_frab_2b_mean, label='RT BF  gain EST (EGT) -- frab -- b=2', linestyle='--')
    plt.plot(snr_db, bf_gain_rt_egt_frab_1b_mean, label='RT BF gain EST (EGT) -- frab -- b=1', linestyle='--')
