...
def plot_paths(id_scene, chn_paths, tx_transceiver, rx_transceiver):
    print('chn_paths:\n', chn_paths)
    print('tx_transceiver:\n', tx_transceiver)
    print('rx_transceiver:\n', rx_transceiver)
...
def plot_devices(id_scene, transceivers):
    for transceiver_id, transceiver in transceivers.items():
        posx = float(transceiver['posx'])
        posy = float(transceiver['posy'])
        device_type = transceiver['type']
        device_label = transceiver['label']
        pathgain = str(' pg:') + str(transceiver['pathgain'])
        if device_type == 'tx':
            plt.plot(posx, posy, 'rs')
        else:
            plt.plot(posx, posy, 'bs')
        plt.text(posx, posy, device_type+device_label+pathgain)
    plt.title(str(id_scene))
    plt.show()
...
def plot_cb_pattern(tx_array, cb):
    ...
    wavelength = tx_array.wavelength
    d = tx_array.element_spacing
    k = 2 * np.pi * (1/wavelength)
