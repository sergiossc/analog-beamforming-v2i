...
if __name__ == '__main__':
    ...
    for pathfile_id, pathfile in pathfiles.items():
        ...
        samples_random_seed = d['trial_random_seed']
        ...
        if initial_alphabet_opt == 'random' and distortion_measure_opt == 'mse':
            last_k = ''
            for k in mean_distortion_by_round.keys():
                last_k = k
            mean_distortion_by_round_list = decode_mean_distortion(mean_distortion_by_round[last_k])
            random_results[str(int(samples_random_seed))] = mean_distortion_by_round_list[-1] 
    ...
    #---------------------------random-----------------------------------------------
    random_results = random_results.items()
    print ('len(random_results): ', len(random_results))
    random_samples_v = np.array([float(v[1]) for v in sorted(random_results)])
    interval_list.append(get_confidence_interval(random_samples_v, t))
    ...
    x_labels = ["katsavounidis", "xiaoxiao", "sa", "unitary", "random from samples", "random"]
