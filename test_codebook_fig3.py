...
if __name__ == "__main__":
    ...
    plt.show()
##
    fig, ax = plt.subplots(2, int(num_of_trials/2),  subplot_kw={'projection': 'polar'}, figsize=(50, 5))
    ax = ax.flatten()
    #axs = []
    ...
    for i in range(num_of_trials):
        ...
        v_opt = bf_vec_opt['opt'][i]
        label_text.append(f'BF gain')
        linestyle.append(f'solid')

        v_opt_egt = bf_vec_opt['egt'][i]
        label_text.append(f'BF gain(EGT)')
        linestyle.append(f'solid')
        cb = np.column_stack((v_opt, v_opt_egt))

        for l in best_results_d.keys():
            v_est_egt = bf_vec_est_egt[l][i]
            print (np.shape(v_est_egt))
            label_text.append(f'BF gain est, L={l}')
            linestyle.append(f'dashed')
            cb = np.column_stack((cb, v_est_egt))
...
##
        v_dft = bf_vec_dft[i]
        label_text.append(f'DFT, L={n_tx}')
        linestyle.append(f'solid')
...
## 
##
        plot_beamforming_from_codeword(cb, label_text, linestyle, ax[i])
    #axs = np.reshape(axs, (int(num_of_trials/2),2))
    #for k in range(int(num_of_trials/2)):
    #    for l in range(2):
    #        ax[k,l] = axs[k,l]
    #plt.legend(loc='best')
    plt.show()
